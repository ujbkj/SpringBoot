...
public class SysUser extends BaseEntity
{
     ...
     * 创建餐饮店标识
     */
    public void creationEateryUid() {
        String uuid = new Date() + UUID.randomUUID(false).toString();
        int max=10;
        int min=0;
        Random random = new Random();
        int s = random.nextInt(max)%(max-min+1) + min;
        this.eateryUid = DigestUtils.md5DigestAsHex(uuid.getBytes()).substring(s, (10 + s));
    }
    ...
    public Integer getEateryType() {
        return eateryType;
    }

    public void setEateryType(Integer eateryType) {
